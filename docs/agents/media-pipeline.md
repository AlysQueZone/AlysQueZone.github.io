...
2. Добавить `[[lots]]` в манифест: `slug`, `title`, `price` (стартовая), `video_url`. Рекомендуемый `title` — `just lots-title privets/<файл>.mp4`: скрипт берёт ник из имени файла и сверяет с реестром ников (`content/chatters.toml`), отдавая канон. Дальше правило реестра (`docs/agents/chatters.md`): писать канон, при резолве дописать алиас; кандидатов несколько — не гадать, спросить человека.
