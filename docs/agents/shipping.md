...
Перед деплоем — контент лотов: `just lots-sync` (из `content/lots.toml`); манифест не применяется автоматически на merge, в отличие от миграций, а сайт собирается из БД. Дальше две ступени:
