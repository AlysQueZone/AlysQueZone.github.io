...
1. Бэкенд: мерж в `main` — Supabase сам применяет новые миграции. Проверка: MCP `list_migrations` — все новые версии в списке. Ошибка — читаем текст, чиним файл неприменённой миграции и пушим снова (очередь встаёт целиком).
