...
Правила записи:
  ...
  - aliases и note скрипт НИКОГДА не трогает — их дописывает агент вручную;
...
# добавляются идемпотентно по login/id. Поля `aliases` и `note` скрипт не трогает:
# агенту можно и нужно дописывать туда варианты написания (кириллица, опечатки) и
# заметки (напр. «несколько аккаунтов одного человека»). `id` — числовой Twitch user id,
# если источник дал. Порядок записей задаёт скрипт (по login); править руками — только
# `aliases`/`note`.\
"""
...
def render(chatters):
    ...
    for c in chatters:
        ...
        if c.get("note"):
            blocks.append("note = %s" % toml_str(c["note"]))
        if c.get("id"):
        ...
        out.append(
            {
                ...
                "note": str(c["note"]) if c.get("note") else "",
