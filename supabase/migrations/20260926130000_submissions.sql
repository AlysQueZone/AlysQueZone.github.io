...
-- а автора, статус и связи считает BEFORE-триггер.
--
-- Решения (тикеты 04/05, правка владельца 2026-09-26): статусы
-- new/accepted/rejected/duplicate меняет только агент/админ через service_role;
-- лимит — не больше 10 открытых заявок на чатерса; ссылка принимается как есть —
-- её пригодность смотрит человек при приёме, поэтому проверок схемы (https) и
-- площадки (allowlist) нет, ограничена только длина.
...
-- 2. BEFORE INSERT: автор из сессии, ссылка — как ввёл человек; серверные поля
...
begin
  ...
  -- Ссылка: принимаем как есть (формат/площадку смотрит человек при приёме);
  -- сервер гарантирует только непустоту и длину.
  if NEW.video_url is null then raise exception 'video url required'; end if;
  NEW.video_url := btrim(NEW.video_url);
  if NEW.video_url = '' then raise exception 'video url required'; end if;
  if length(NEW.video_url) > c_url_max then
