-- [Стройка video-s3] Запрет самовыкупа: свой лот купить нельзя.
-- Переписывает enforce_purchase_rules() (create or replace — идемпотентно):
-- плюс guard owner_uid == buyer. Первый покупатель (owner NULL) проходит.
-- Применение: db push или мерж в main (Deploy to production).
