...
- 2026-09-07 (тема + топбар): колокольчик и уведомление перекрашены в цвета сайта (крем void/panel, кайма pivko/rind — как панель истории); топбар в 3 зоны (лого | гамба+баланс | колокольчик+ник+выйти), центрирование по max-w-6xl сохранено.
- 2026-09-07 (перекуп): кнопка «Забрать» несла только id/title/price — модалка не знала звук лота и играла хлопки. Теперь тянем `data-lot-audio` (+ `data-lot-owner`) из витринной кнопки. Плюс: событие `alysque:bought` чистит историю колокольчика и закрывает окошки по этому лоту (не актуальны после обратного выкупа).
Blocked by: none
Owns (другие тикеты эти файлы не трогают): новый `src/lib/outbid-notice.ts` (переименован из `outbid-alert.ts` под глоссарий: «уведомление», не «алерт»), `src/layouts/BaseLayout.astro` (только подключение), новый `public/sounds/outbid.mp3`.
