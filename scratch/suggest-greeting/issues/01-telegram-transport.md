...
- **Рекомендация — DB-нативный путь**: `after insert`-триггер на таблице заявок → `security definer`-функция в непубличной схеме `private` → `net.http_post` (pg_net) прямо в `api.telegram.org/.../sendMessage`. Токен и chat_id — в Supabase Vault (расширение уже установлено в проекте; доступно на всех тарифах, включая Free; секреты кладутся руками через Dashboard → Vault — штатный путь), в git не попадают; всё уезжает обычной миграцией из `main`, без новых артефактов деплоя.
